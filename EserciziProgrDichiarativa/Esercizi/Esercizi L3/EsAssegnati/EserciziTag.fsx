...
Riconsideriamo le seguenti definizioni viste a lezione (file tag.fsx):
...
Una figura e' well_formed (ben definita) se nessuna delle sue dimensioni e' negativa.
...
let isWellFormed (fig:figura) = 
    match fig with
    | Rettangolo(b,h) when (b>=0. && h>=0.) -> true  
    | Quadrato l when l>=0. -> true  
    | Triangolo(b,h) when (b>=0. && h>=0.) -> true
    | _ -> false

let areaOpt figura = 
    match isWellFormed figura with
    | false -> None
    | true -> 
        match figura with
        | Rettangolo(b,h) -> Some(b*h)
        | Quadrato l -> Some(l*l)
        | Triangolo(b,h) -> Some(b*h/2.)
...
let sommaArea (aFig:figura, bFig:figura) = 
    match (areaOpt aFig, areaOpt bFig) with
...
let rec sommaAreaList (figs: figura list) = 
    match figs with
    | [] -> 0.
    | x::xs -> 
        match areaOpt x with
        | None -> sommaAreaList xs
        | Some area -> area+sommaAreaList xs
