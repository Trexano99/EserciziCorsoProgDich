...
** Esercizio difficile, vedi es. 11.7 del libro [HR].
   Occorre usare la funzione sumSeq definita in  seq.fsx (script discusso a lezione)  **
...
let sift number = Seq.filter (fun x -> x%number <> 0)
...
let resSq1 =  Seq.take 10 sq1 |> Seq.toList
resSq1
let resSq2 = Seq.take 15 sq2 |> Seq.toList
resSq2
...
let rec sieve sq = 
    seq{
        let x0 = Seq.head sq
        yield x0
        yield! sieve (sift x0 (Seq.tail sq))
    }

let nat2 = Seq.initInfinite (fun x -> x+2)

let sieves = sieve nat2 |> Seq.take 10 |> Seq.toList
...
let rec sieveC sq = 
    seq{
        let x0 = Seq.head sq
        yield x0
        yield! sieve (siftC x0 (Seq.tail sq))
    }
...
let firtsRes = Seq.item 1000 primesC
//val it : int = 7927
let secondRes =Seq.item 1000 primesC
//val it : int = 7927
...
let t (x, k) = 
    (pown x k) / (Seq.fold (fun res x -> res * (float)x) 1. [1 .. k] )

let f x k = 
    t(x,k)

let rec taylor x value prec = seq{
    let valore = (f x value) + prec
    yield valore
    yield! taylor x (value+1) valore
}

let apprTaylor x = seq{
    yield! taylor x 0 0.
}

let aTestRes = apprTaylor 1.0 |> Seq.take 10   |> Seq.toList
let bTestRes = apprTaylor 2.5 |> Seq.take 10   |> Seq.toList


let apprExp (x:float) (delta:float) :float = 
    let taylor = apprTaylor x
    Seq.item( Seq.find(fun x -> abs((Seq.item (x+1) taylor) - Seq.item(x) taylor) < delta ) (Seq.initInfinite id)) taylor

    

let t1 = apprExp 1.0 0.01  
// val it : float = 2.708333333

let t2 = apprExp 1.0 0.0001 
// val it : float = 2.718253968

let t3 = apprExp 1.0 0.0000001 
// val it : float = 2.718281801

let t4 = apprExp 2.5 0.0000001 
// val it : float = 12.18249394
