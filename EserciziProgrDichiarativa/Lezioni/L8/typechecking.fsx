...
// sarebbe stato meglio usare singleton tagged value, ma poi bisgna fare PM ...
// type envt = Env of Map<vname,int> ;;
...
let value  = VI of int | VB of bool

and make env from vname to value
*)
...
// more informative to give meaningful errors with Result
...
// we can change the defaul setting of 100 tests, using fsCheck config record
...
let prop_pres_naive e =
    ...
    (t <> None) ==>
        (lazy  (t = (tpchko (eval e Map.empty)  Map.empty)))
...
// Very small expressions, so not a great test. We can do a bit better by writing our own generator
...
//  Still, we can restate the preservation property quantifying only
...
let prop_pres =
    Prop.forAll wellTypedTermsOf // forall e: tau, ...
     (fun e ->
        ...
        tpchko e Map.empty = tpchko v  Map.empty)
